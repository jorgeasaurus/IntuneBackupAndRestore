function Invoke-IntuneBackupAutopilotDeploymentProfileAssignment {
    ...
    $winAutopilotDeploymentProfiles = Invoke-MgGraphRequest -OutputType PSObject -Uri "$ApiVersion/deviceManagement/windowsAutopilotDeploymentProfiles" | Get-MGGraphAllPages
	...
	if ($winAutopilotDeploymentProfiles) {
		...
		foreach ($winAutopilotDeploymentProfile in $winAutopilotDeploymentProfiles) {
			$assignments = Invoke-MgGraphRequest -OutputType PSObject -Uri "$ApiVersion/deviceManagement/windowsAutopilotDeploymentProfiles/$($winAutopilotDeploymentProfile.id)/assignments" | Get-MGGraphAllPages
