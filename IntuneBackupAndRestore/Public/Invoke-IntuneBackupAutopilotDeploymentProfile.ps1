function Invoke-IntuneBackupAutopilotDeploymentProfile {
    ...
    $winAutopilotDeploymentProfiles = Invoke-MgGraphRequest -OutputType PSObject -Uri "https://graph.microsoft.com/$ApiVersion/deviceManagement/windowsAutopilotDeploymentProfiles" -OutputType PSObject | Select-Object -ExpandProperty Value
	...
	if ($winAutopilotDeploymentProfiles) {
		...
		foreach ($winAutopilotDeploymentProfile in $winAutopilotDeploymentProfiles) {
			...
			$winAutopilotDeploymentProfileObject = Invoke-MgGraphRequest -OutputType PSObject -Uri "https://graph.microsoft.com/$ApiVersion/deviceManagement/windowsAutopilotDeploymentProfiles/$($winAutopilotDeploymentProfile.id)"
